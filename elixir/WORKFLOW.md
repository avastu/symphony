...
`Human Review` is a durable wait boundary. Returning from `Rework` to `Human Review` establishes a fresh semantic review baseline: PR head/checkpoint IDs, unresolved actionable review-thread IDs, relevant check annotation/failure IDs, and the latest post-handoff human change-request comment ID. A `## Codex Workpad` update is the review handoff boundary for Linear comment commands; older `Revise plan:`, `Approved with change:`, `Answer:`, or `Retry` comments are stale for the new review packet. Generated Symphony comments such as `## Codex Workpad`, `## Symphony Uploaded Artifacts`, and `## Symphony Review Loop Guard` are not review feedback sources. Routine Linear/workpad timestamp churn, status-only handoffs, publish-path blockers, blocked packets, resolved/outdated review comments, and reviewer infrastructure failures without code feedback must not move the issue to `Rework`. Automatic `Rework` transitions must name the concrete semantic trigger ID in their audit reason.
...
3. Route to the matching flow:
   ...
   - `Human Review` -> wait for event/checkpoint wakeups; if a new unresolved actionable review-thread ID, relevant check annotation/failure ID, policy-triggering PR head/checkpoint ID, or explicit human change-request comment after the latest workpad handoff such as `Revise plan:`, `Approved with change:`, `Answer:`, or `Retry` is detected, move to `Rework` and address it. Do not move `ready_for_review` packets to `Rework` solely because Linear `updatedAt` changed from a status-only handoff or automation note.
...
2. Do not repeatedly dispatch stable `Human Review` issues. The control plane may scan semantic review/checkpoint metadata; if it detects a new actionable trigger ID such as an explicit post-handoff Utsav change command, relevant PR check annotation/failure, new unresolved review feedback, or policy-triggering PR head SHA change, it must move the issue to `Rework` before dispatching the agent to address that trigger. Status-only comments on a workpad `State: ready_for_review` packet are non-action review boundaries and must not trigger `Rework`.
3. If a rapid `Human Review -> Rework -> Human Review -> Rework` pattern is detected without a new actionable trigger ID, stop dispatching and surface a human decision item instead of continuing the loop.
...
8. After the workpad reflects the merge result, post one fresh concise completion handoff comment with the merge/validation outcome.
