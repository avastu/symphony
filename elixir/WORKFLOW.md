...
3. Route to the matching flow:
   ...
   - `Human Review` -> wait for event/checkpoint wakeups; if a new unresolved actionable review-thread ID, relevant check annotation/failure ID, policy-triggering PR head/checkpoint ID, or explicit human change-request comment such as `Revise plan:`, `Approved with change:`, `Answer:`, or `Retry` is detected, move to `Rework` and address it. Do not move `ready_for_review` packets to `Rework` solely because Linear `updatedAt` changed from a status-only handoff or automation note.
